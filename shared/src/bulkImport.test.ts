...
describe('parseBulkImport (CSV)', () => {
  it('正常なCSVを正しくパースできる', () => {
    ...
    expect(result.records[0].data).toMatchObject({ name: 'Alice', age: 30, active: true });
    expect(result.records[1].data).toMatchObject({ name: 'Bob', age: 25, active: false });
  });
  ...
  it('ダブルクォートで囲まれたカンマを含む値を正しく扱う', () => {
    ...
    const result = parseBulkImport(csv, 'csv', model);
    expect(result.parseError).toBeUndefined();
    expect(result.records[0].data.name).toBe('Smith, John');
  });
});
...
describe('parseBulkImport (TSV)', () => {
  it('正常な TSV をパースできる', () => {
    ...
    expect(result.parseError).toBeUndefined();
    expect(result.records[0].data).toMatchObject({ name: 'Charlie', age: 22, active: true });
  });
});
...
describe('parseBulkImport (JSON)', () => {
  it('正常な JSON 配列をパースできる', () => {
    ...
    expect(result.parseError).toBeUndefined();
    expect(result.records[0].data).toMatchObject({ name: 'Dave', age: 35, active: false });
  });
...
describe('formatErrorLog', () => {
  it('RowError 配列を TSV 形式に整形する (modelなし)', () => {
    const errors = [
  ...
  it('RowError 配列を TSV 形式に整形する (modelあり)', () => {
    const errors = [
      { row: 1, field: 'name', message: 'is required (NOT NULL)', recordData: { name: '', age: 30, active: true } },
      { row: 3, field: 'age', message: 'must be number', recordData: { name: 'Bob', age: 'invalid', active: false } },
    ];
    const log = formatErrorLog(errors, model);
    const lines = log.split('\n');
    expect(lines[0]).toBe('行番号\tname\tage\tactive\t_errors');
    expect(lines[1]).toBe('1\t\t30\ttrue\tname: is required (NOT NULL)');
    expect(lines[2]).toBe('3\tBob\tinvalid\tfalse\tage: must be number');
  });
});
